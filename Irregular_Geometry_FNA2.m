...
% FNA2-type Computation - Irregular Geometry
% Ben Adcock - Mohsen Seifi, Simon Fraser University, Department of Math

%   -------------------------------------------------------------------   % 
Err_Ylim = [1e-16, 1e0];    Err_YTick = 10.^(-16:4:0);
coef_Ylim = [1e-1, 1e12]; coef_YTick = 10.^(0:3:9);
...
MVect = gamma*NVect;
K = 10*NMax;    ErrGrid = linspace(-omega, omega, K)'; fGrid = f(ErrGrid);
...
for j = 1:NL
    ...
    N = NVect(j, 1);    
    E = (1/sqrt(2))*SystemMatrixLeg(0:1:N-1, ErrGrid);  
    
    M = MVect(j, 1);    Grid = linspace(-omega, omega, M)'; 
    
    l = sqrt(2*omega/M)*f(Grid);
    
    Phi = sqrt(omega/M)*SystemMatrixLeg(0:1:N-1, Grid);
    [U, S, V] = svd(Phi);
...
saveas(figure(1), strcat(name, '_IG_FNA2_ASVD1_err'), 'epsc');
saveas(figure(1), strcat(name, '_IG_FNA2_ASVD1_err.fig'));
%% ASVD2_Err plot
...
saveas(figure(2), strcat(name, '_IG_FNA2_ASVD2_err'), 'epsc');
saveas(figure(2), strcat(name, '_IG_FNA2_ASVD2_err.fig'));
%% ASVD1_Coef plot
...
saveas(figure(3), strcat(name, '_IG_FNA2_ASVD1_coef'), 'epsc');
saveas(figure(3), strcat(name, '_IG_FNA2_ASVD1_coef.fig'));
%% ASVD2_Coef plot
...
saveas(figure(4), strcat(name, '_IG_FNA2_ASVD2_coef'), 'epsc');
saveas(figure(4), strcat(name, '_IG_FNA2_ASVD2_coef.fig'));
